...
type SkillTopicsTableProps = {
    ...
    onOpenAddEntryWithContext: (params: {
        skillLabel: string;
        topicLabel: string;
    }) => void;
};

export function SkillTopicsTable({ topics, skillId, onOpenPost, onOpenAddEntryWithContext }: SkillTopicsTableProps) {
    const { toggleTopic } = useTopicCompletionToggle({
        openWithLockedContext: onOpenAddEntryWithContext,
    });
								...
								<tr key={topic.id} className="border-b border-(--bdr) transition-colors last:border-b-0 hover:bg-(--topics-table-row-hover)">
									<td className="px-[14px] py-3 align-middle">
										<div className="flex items-center justify-center">
											<button
												type="button"
												onClick={() => toggleTopic({ skillId, topicId: topic.id })}
												disabled={topic.done}
												className={`flex h-4 w-4 items-center justify-center rounded-[4px] border-[1.5px] transition-colors ${topic.done ? "cursor-auto border-(--rose) bg-(--rose-dim)" : "cursor-pointer border-(--bdr2) bg-transparent hover:border-(--rose)"}`}
												aria-label={topic.done ? "Completed topic" : "Mark complete"}
												title={topic.done ? "Completed topic" : "Mark complete"}
											>
												<svg
													width="8"
													height="8"
													viewBox="0 0 10 10"
													fill="none"
													className={topic.done ? "opacity-100" : "opacity-0"}
													aria-hidden="true"
												>
									...
									<td className="px-[14px] py-3 align-middle">
										<span className={`whitespace-nowrap text-[13px] font-medium ${topic.done ? "text-(--tx3) line-through" : "text-(--tx)"}`}>
											{topic.name}
										</span>
									</td>
									<td className="px-[14px] py-3 align-middle">
										<span className={`font-mono text-[11px] whitespace-nowrap ${topic.done ? "text-(--tx2)" : "text-(--tx3)"}`}>
											{topic.finishedOn ?? "—"}
										</span>
									</td>
									<td className="px-[14px] py-3 align-middle">
										{topic.done && topic.asset ? (
											<a
												href={topic.asset.url}
												target="_blank"
												rel="noreferrer"
												className="inline-flex items-center gap-1 rounded-[6px] border border-(--rose-dim) bg-(--rose-dim) px-2 py-[3px] text-[11px] text-(--rose) no-underline transition-colors hover:border-[color-mix(in_srgb,var(--rose),transparent_70%)] hover:bg-(--rose-glow)"
											>
									...
									<td className="px-[14px] py-3 text-center align-middle">
										{topic.done && topic.postId !== null ? (
											<button
												type="button"
												onClick={() => onOpenPost(topic.id)}
												className="inline-flex items-center gap-1 rounded-[6px] border border-(--rose-dim) bg-(--rose-dim) px-[10px] py-1 text-[11px] font-semibold text-(--rose) transition-colors hover:border-[color-mix(in_srgb,var(--rose),transparent_70%)] hover:bg-(--rose-glow) hover:cursor-pointer"
											>
												<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
