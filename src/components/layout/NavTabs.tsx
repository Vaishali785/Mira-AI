import { dashboardTabs } from "@/data/mock-data";
import { DashboardTab } from "@/types/app-types";

type NavTabsProps = {
    activeTab: DashboardTab;
    onTabChange: (tab: DashboardTab) => void;
};

export function NavTabs({ activeTab, onTabChange }: NavTabsProps) {
    return (
        <div className="w-full overflow-x-auto py-1 no-scrollbar">
            <div className="flex min-w-max items-center gap-5 sm:gap-6">
                {dashboardTabs.map((tab) => (
                    <button
                        key={tab}
                        type="button"
                        onClick={() => onTabChange(tab)}
                        className={`relative px-[2px] py-[8px] text-[12px] font-medium transition-colors duration-200 ${activeTab === tab
                            ? "text-[var(--tx)]"
                            : "text-[var(--tx3)] hover:text-[var(--tx2)]"
                            }`}
                    >
                        {tab}
                        {activeTab === tab ? <span className="absolute bottom-0 left-0 right-0 h-[2px] rounded-t-[2px] bg-[var(--rose)]" /> : null}
                    </button>
                ))}
            </div>
        </div>
    );
}
