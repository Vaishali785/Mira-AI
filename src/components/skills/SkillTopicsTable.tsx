...
export function SkillTopicsTable({ isLight, topics, onOpenPost, onToggleTopic }: SkillTopicsTableProps) {
    return (
        <section
            className="overflow-hidden rounded-[12px] border border-(--bdr2) transition-colors duration-300"
            style={{
                background: isLight
                    ? "radial-gradient(ellipse 80% 80% at 5% 95%, var(--rose-dim) 0%, transparent 55%), radial-gradient(ellipse 50% 50% at 95% 5%, rgba(248,244,240,0.3) 0%, transparent 50%), var(--card)"
                    : "radial-gradient(ellipse 80% 80% at 5% 95%, var(--rose-dim) 0%, transparent 55%), radial-gradient(ellipse 50% 50% at 95% 5%, rgba(255,255,255,0.03) 0%, transparent 50%), var(--card)"
            }}
        >
            <div className="w-full overflow-x-auto">
                <table className="min-w-[760px] w-full border-collapse">
                    ...
                    <tbody>
                        {topics.map((topic) => (
                            <tr key={topic.id} className={`border-b border-(--bdr) transition-colors last:border-b-0 ${isLight ? "hover:bg-black/[0.025]" : "hover:bg-white/[0.025]"}`}>
